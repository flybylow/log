...
- **CI:** Pushes to `main` / `master` run `.github/workflows/ci.yml` on Node 22: **`npm ci`**, **`npm run build`**, **`npm test`**, and **`npm run test:frontend`**.
...
3. **This repo’s `package.json`** — Combell requires **`build`** and **`serve`** scripts. Here, **`build`** runs **`npm ci`** at the repo root (so **`node_modules`** matches **`package-lock.json`**, including **`typescript`**), then **`build:frontend`** then **`build:server`** (`node node_modules/typescript/bin/tsc` → `dist/`), and **`serve`** runs **`node dist/server.js`**. Hosts that only run **`npm run build`** without a separate root **`npm install`** need this root **`npm ci`** step. For a faster rebuild when you already installed deps locally, use **`npm run build:quick`**. Do not replace `build` with **`build:server`** alone — the SPA at **`GET /`** would be missing. Avoid bare **`npx tsc`** without a local **`typescript`** install: npm may install the wrong **`tsc`** package from the registry.
...
| **`Cannot find module '…/node_modules/typescript/bin/tsc'`**, **`sh: 1: tsc: not found`**, **“not the tsc command you are looking for”**, or **`npx` installs `tsc@2.x`** | The **`build`** script starts with **`npm ci`** so the **root** **`node_modules`** is installed before **`build:server`** (some pipelines only run **`npm run build`** and never **`npm install`** at the repo root). **`typescript`** and compile-time **`@types/*`** are in **`dependencies`** so production-only installs still get **`tsc`**. Local dev still uses **`tsx`** / **`supertest`** from **devDependencies**. Avoid bare **`npx tsc`** when TypeScript is missing — npm may install the wrong **`tsc`** package. |
