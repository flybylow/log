...
| **`Cannot find module '…/node_modules/typescript/bin/tsc'`**, **`sh: 1: tsc: not found`**, **“not the tsc command you are looking for”**, or **`npx` installs `tsc@2.x`** | Hosts that run **`npm ci --omit=dev`** (production-only) omit **devDependencies** by default. **`typescript`** and the **`@types/*`** packages needed to compile **`src/`** are listed under **`dependencies`** so **`npm run build`** still runs **`node node_modules/typescript/bin/tsc`** after a production install. Local dev still uses **`tsx`** / **`supertest`** from **devDependencies**. Avoid bare **`npx tsc`** when TypeScript is missing — npm may install the wrong **`tsc`** package. |
