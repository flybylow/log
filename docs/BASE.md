...
- **Frontend:** `frontend/` is a Vite + React + Tailwind SPA: **Turtle** from `GET /graph` is parsed and shown as a **React Flow** graph; **`/api/timeline`** drives a **timeline widget**; **Send EPCIS event** uses construction samples from `embeddedExamples.ts` (aligned with repo `examples/`). **`npm run build`** runs `build:frontend` then **`tsc`**. Express serves **`GET /`** from `frontend/dist` when present; otherwise **`503`** with a build hint.
